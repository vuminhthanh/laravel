                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            @foreach($users as $user)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('user-edit',$user->id)}}" class="btn btn-primary"><i class="fas fa-edit"></i>&nbsp;Chỉnh sửa</a>
                                        <button class="btn btn-danger" onclick="deleteUsers(this,'{{$user->id}}')"><i class="fas fa-trash-alt"></i>&nbsp;Xóa</button>

                                        @if($user->user_status == 'on')
                                            <button class="btn btn-success" onclick="userStatus(this,'{{$user->id}}','off')"><i class="fas fa-eye-slash"></i>off</button>
                                        @else
                                            <button class="btn btn-success" onclick="userStatus(this,'{{$user->id}}','on')"><i class="fas fa-eye"></i>on</button>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
