...
class BlogController extends Controller
{
    ...
    public function postBlogs(Request $request)
    {
        if (isset($request->delete)) {
            try {
                ...
                return response(['status' => 'success', 'title' => 'Thành công', 'content' => 'Blog đã bị xóa']);
            } catch (\Exception $e) {
                return response(['status' => 'error', 'title' => 'Thất bại', 'content' => 'Không xóa được blog']);
            }
        } elseif (isset($request->blog_status)) {
            try {
                Blogs::where('id', $request->id)->update($request->all());
                return response(['status' => 'success', 'title' => 'Thành công', 'content' => 'Trạng thái Blog đã thay đổi']);
            } catch (\Exception $e) {
                return response(['status' => 'error', 'title' => 'Thất bại', 'content' => 'Không thể thay đổi trạng thái blog']);
            }
        }
    }
    ...
    public function postBlogsAdd(Request $request)
    {
        try {
            ...
            return response(['status' => 'success', 'title' => 'Thành công', 'content' => 'Đã thêm blog']);
        } catch (\Exception $e) {
            return response(['status' => 'error', 'title' => 'Không thành công', 'content' => 'Không thêm được blog']);
        }
    }
    ...
    public function postBlogsEdit(Request $request, $blogId)
    {
        try {
            ...
            return response(['status' => 'success', 'title' => 'Thành công', 'content' => 'Đã cập nhật blog']);
        } catch (\Exception $e) {
            echo $e;
